...
type ImpiantoFotovoltaico = {
  ...
  descrizione: string
  costo: number,
  minimoPotenza: number,
  massimoPotenza: number,
  tensione: string,
  novita: boolean
}
...
const EditPacchetti: NextPageWithLayout = () => {
  ...
  const onSubmit: SubmitHandler<ImpiantoFotovoltaico> = async (formdata: any) => {
    ...
    mpApi.services.actions
      ...
      .then((response: any) => {
        ...
        push('/pacchetti/fotovoltaico/edit?id=' + response.data.id)
      })
      .catch((reason: any) => {
        alert({
          id: new Date().toISOString(),
        ...
        Object.keys(reason.data.errors).forEach((field: string) => {
          setError(
            field as 'id' | 'nome' | 'descrizione' | 'costo' | 'novita',
            {
              type: 'custom',
  ...
  return item ? (
        ...
        <div>
          ...
          <div className='mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6'>
            ...
            <FormInput
              ...
              autoComplete='nome'
              aria='Minimo potenza'
              label='Minimo potenza'
              defaultValue={item?.minimoPotenza ?? ''}
            />
            <FormInput
              className='sm:col-span-3'
              {...register('nome', { required: true })}
              errorMessage={renderError(errors['nome'])}
              autoComplete='nome'
              aria='Massima potenza'
              label='Massima potenza'
              defaultValue={item?.massimoPotenza ?? ''}
            />
            <CheckboxInput
    ...
    <FourOFour
      ...
      description='Il contenuto che hai richiesto è stato rimosso oppure non esiste.'
    />
  )
}

EditPacchetti.getLayout = function getLayout (page: ReactElement) {
  return <SidebarLayout title='Pacchetti di manutenzione - fotovoltaico'>{page}</SidebarLayout>
}
