...
export class User {
	...
	canCreate(type: string) {
		return (
			...
			(this.data && this.abilities.has(type + "-create"))
		);
	}
	canDelete(type: string) {
		return (
			this.roleIS(ADMIN_ROLE) ||
			(this.data && this.abilities.has(type + "-delete"))
		);
	}
	canEdit(type: string) {
		return (
			this.roleIS(ADMIN_ROLE) ||
			(this.data && this.abilities.has(type + "-edit"))
		);
	}
	canPublish(type: string) {
		return (
			this.roleIS(ADMIN_ROLE) ||
			(this.data && this.abilities.has(type + "-publish"))
		);
	}
