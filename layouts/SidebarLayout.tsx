...
function SidebarLayout({title, children}: any) {
    ...
    useEffect(() => {
        ...
        const menu = [
            {slug: 'servizi', label: 'Sevizi', icon: <DownloadIcon/>, open: false},
            {slug: 'manutenzione', label: 'Manutenzione', icon: <TicketIcon/>, children: [], category: 10},
            {slug: 'pacchetti', label: 'Pacchetti', icon: <OfficeBuildingIcon/>, open: false},
            {slug: 'ordini', label: 'Ordini', icon: <OfficeBuildingIcon/>, open: false},
            {slug: 'collaboratori', label: 'Collaboratori', icon: <StarIcon/>, open: false},
            {slug: 'user', label: 'Clienti', icon: <UsersIcon/>, open: false},
            {slug: 'preventivi', label: 'Preventivi', icon: <LibraryIcon/>, open: false},
        ].filter((item: any) => {
        ...
        <>
            ...
            <div className={
                ...
                >
                    ...
                    <div>
                        <NavigationMenu collapsed={!sidebar} className="mt-2 text-white bg-indigo-800" menu={sezioni}
                                        onClick={(item: MenuItem) => {
                                            console.log(item)
                                        }}/>
                    </div>
                    <div className='absolute w-full bottom-0 right-0 left-0 text-center py-2 bg-black/40'>
                        <button className='w-5 h-5 mx-auto' onClick={() => setSidebar(!sidebar)}><ChevronDoubleLeftIcon
                            className={`${!sidebar ? 'transform rotate-180' : ''} duration-500 transition ease-in-out`}/>
                        </button>
                    </div>
                </div>
                {/* Sidebar ends */}
